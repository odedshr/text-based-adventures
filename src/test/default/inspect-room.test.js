...
async function shouldDescribeTheRoom(location) {
    const puzzle = await import (`../../../docs/js/scenarios/mansion-escape/rooms/${location.replace(/ /g, '-')}.js`);
    const { variables, actions, strings } = puzzle.default;
    const gameDefinition = {
        variables: { ...variables, user: { location } },
